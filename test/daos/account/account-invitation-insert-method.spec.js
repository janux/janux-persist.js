...
describe("Testing accountInvitation dao insertMethod methods", function() {
	[DataSourceHandler.MONGOOSE, DataSourceHandler.LOKIJS].forEach(function(dbEngine) {
		...
		beforeEach(function(done) {
			...
			accountInvitationDao.removeAll().then(function() {
				done();
			});
		});
				...
				accountInvitationDao
					...
					.then(
						function(insertedRecord) {
							...
							accountInvitationDao.findOne(insertedRecord.id).then(function(resultQuery) {
		...
		describe("When inserting a record that has an id", function() {
			it("It should send an error", function(done) {
				...
				accountInvitationDao.insert(accountInvitation).then(function(result) {
					accountInvitationDao.insert(result).then(
						function(result2) {
