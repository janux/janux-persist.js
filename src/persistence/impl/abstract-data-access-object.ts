 ...
 * be inserted, updated or deleted. But the dao provides a way to tell him how to validate and when to validate.
 ...
 * 5.- This dao, for the moment, only works for no-sql databases ( currently lokijs and mongodb).
 * 6.- There is only one dao for one collection in a no-sql database.
 * 7.- Given that every db engine is different, the janux-persistence dao has the following considerations.
 * a) Lokijs and mongodb db assigns an unique id for every document. In lokijs the id is stores in a attribute called
 * “$loki” of the type number. In mongodb the id is stored in an attribute called “_id” and has a custom value type.
 ...
 * 8.- Most dao methods return a bluebird promise. Where Promise.resolve() indicates the operation the dao executed
 * were successful. Any Promise.reject() or error indicates something went wrong.
 ...
 * So, any dao implemented by janux-persistence does not have the responsibility to validate the content against
