...
export class AccountInvitationValidator {
	...
	public static validateAccountInvitation(accountInvitationEntity: AccountInvitationEntity): ValidationErrorImpl[] {
		this._log.debug(
			"Call to validateAccountInvitation with accountInvitationInvitationEntity: %j:",
	...
	public static validateResultQueryBeforeBdOperation(
		accountInvitations: AccountInvitationEntity[],
		reference: AccountInvitationEntity
	): ValidationErrorImpl[] {
		this._log.debug(
			"Call to validateResultQueryBeforeBdOperation with accountInvitations: %j reference: %j",
			accountInvitations,
			reference
		);
		const errors: ValidationErrorImpl[] = [];
		if (accountInvitations.length > 0) {
			if (accountInvitations[0].code === reference.code) {
				errors.push(new ValidationErrorImpl("code", this.ANOTHER_CODE, reference.accountId));
			}
		}
