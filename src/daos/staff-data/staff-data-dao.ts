...
export class StaffDataDao extends AbstractDataAccessObjectWithAdapter<StaffDataEntity, string> {
	...
	public removeByIdContact(idContact: string): Promise<any> {
		return this.findOneByIdContact(idContact).then((result: StaffDataEntity) => {
			if (!_.isNil(result)) {
	...
	public removeByIdContactIn(idContacts: string[]): Promise<any> {
		return this.findByIdContactIn(idContacts).then((result: StaffDataEntity[]) => {
	...
	protected validateBeforeInsert(objectToInsert: StaffDataEntity): Promise<ValidationErrorImpl[]> {
		...
		return this.findOneByIdContact(objectToInsert.idContact).then((result: StaffDataEntity) => {
			const errors: ValidationErrorImpl[] = [];
			if (result != null) {
				errors.push(
					new ValidationErrorImpl(
						StaffDataEntityValidator.ID_CONTACT,
						StaffDataEntityValidator.ID_CONTACT_DUPLICATED,
						objectToInsert.idContact
					)
				);
			}
	...
	protected validateBeforeUpdate(objectToUpdate: StaffDataEntity): Promise<ValidationErrorImpl[]> {
		...
		const query = {
			$and: [{ id: { $ne: objectToUpdate.id } }, { idContact: { $eq: objectToUpdate.idContact } }]
		};
		return this.findByQuery(query).then((result: StaffDataEntity[]) => {
			const errors: ValidationErrorImpl[] = [];
			if (result.length > 0) {
				errors.push(
					new ValidationErrorImpl(
