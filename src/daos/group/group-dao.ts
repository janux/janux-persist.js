...
export class GroupDao extends AbstractDataAccessObjectWithAdapter<GroupEntity, string> {
	...
	protected validateBeforeInsert(objectToInsert: GroupEntity): Promise<ValidationErrorImpl[]> {
		return this.findOneByCode(objectToInsert.code).then((resultQuery: GroupEntity) => {
			const errors: ValidationErrorImpl[] = [];
			if (resultQuery != null) {
				errors.push(
					new ValidationErrorImpl(GroupValidator.CODE, GroupValidator.CODE_DUPLICATE, objectToInsert.code)
				);
			}
	...
	protected validateBeforeUpdate(objectToUpdate: GroupEntity): Promise<ValidationErrorImpl[]> {
		const query = {
			$and: [{ id: { $ne: objectToUpdate[this.ID_REFERENCE] } }, { code: { $eq: objectToUpdate.code } }]
		};
		return this.findByQuery(query).then((resultQuery: GroupEntity[]) => {
			const errors: ValidationErrorImpl[] = [];
			if (resultQuery.length > 0) {
				errors.push(
					new ValidationErrorImpl(GroupValidator.CODE, GroupValidator.CODE_DUPLICATE, objectToUpdate.code)
				);
			}
			return Promise.resolve(errors);
		});
