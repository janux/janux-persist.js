...
export class EnvironmentService {
	public static getEnvironmentInfo(): EnvironmentInfo {
		...
		if (
			config["serverAppContext"] &&
			config["serverAppContext"].system &&
			config["serverAppContext"].system.environment &&
			!isBlankString(config["serverAppContext"].system.environment)
		) {
